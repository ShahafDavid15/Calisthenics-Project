...
export default function Profile({ onLogout }) {
  ...
  useEffect(() => {
    ...
    const fetchMembershipHistory = async () => {
      try {
        const response = await apiFetch("/api/purchases/membership-history");
        if (response.ok) {
          const data = await response.json();
  ...
  const isMembershipActive = (endDate) => new Date(endDate) >= new Date();
  ...
  return (
    <div className={classes.container}>
      ...
      <main className={classes.main}>
        <h2 className={classes.title}>פרופיל</h2>
        ...
        <form onSubmit={handleSubmit} className={classes.form} noValidate>
          ...
          <div className={classes.inputContainer}>
            ...
            >
              ...
              {/* values must match DB enum: male | female | other */}
        ...
        <h2 className={`${classes.title} ${classes.sectionGap}`}>היסטוריית מנויים</h2>

        {membershipHistory.length === 0 ? (
          <p className={classes.noHistory}>אין היסטוריית מנויים</p>
        ) : (
              ...
              <tbody>
                {membershipHistory.map((m) => (
                  <tr key={m.id}>
                    ...
                    <td>₪{Number(m.price_with_vat).toLocaleString()}</td>
                    <td>
                      <span className={isMembershipActive(m.end_date) ? classes.badgeActive : classes.badgeExpired}>
                        {isMembershipActive(m.end_date) ? "פעיל" : "פג תוקף"}
                      </span>
                    </td>
                  </tr>
                ))}
