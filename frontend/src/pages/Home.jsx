...
export default function Home({ onLogout, currentUser }) {
  ...
  // Fetch exercise progress when selected exercise changes
  useEffect(() => {
    if (!selectedProgressExercise || !currentUser?.id || isAdmin) return;
    setProgressLoading(true);
    const fetchProgress = async () => {
      try {
        const res = await apiFetch(
          `http://localhost:3002/api/workout-stats/exercise-progress?exercise=${encodeURIComponent(selectedProgressExercise)}`
        );
        if (res.ok) {
          const data = await res.json();
    ...
    fetchProgress();
  }, [selectedProgressExercise, currentUser, isAdmin]);
        ...
        {!loading && !isAdmin && (
          <div className={classes.userWorkouts}>
            ...
            <div className={classes.statsContainer}>
              ...
              {exerciseStats.length > 0 && (
                <>
                  <h3 className={classes.chartTitle}>התקדמות בתרגיל לאורך זמן</h3>
                  <label className={classes.filterLabel}>
                    בחר תרגיל:
                    <select
                      className={classes.filterSelect}
                  ...
                  {selectedProgressExercise && !progressLoading && progressData.length > 0 && (
                    <div className={classes.progressChartWrap}>
                    <ResponsiveContainer width="100%" height={320}>
                      <LineChart
                        data={progressData.map((d) => ({
                          ...d,
                          total_reps: Number(d.total_reps),
                          workout_date: new Date(d.workout_date).toLocaleDateString("he-IL"),
                        }))}
                        margin={{ top: 48, right: 24, left: 8, bottom: 8 }}
                      >
                        <CartesianGrid strokeDasharray="3 3" />
                        <XAxis dataKey="workout_date" tick={{ className: classes.axisTick }} height={50} />
                        <YAxis
                          tick={{ className: classes.axisTick }}
                          width={56}
                          tickMargin={8}
                          domain={[
                            ...
                            (dataMax) => {
                              ...
                              return Math.ceil(n * 1.12) + 4;
                            },
                          ]}
                          allowDecimals={false}
                        />
                        <Tooltip
                          formatter={(value) => [value, "חזרות"]}
                  ...
                  {selectedProgressExercise && !progressLoading && progressData.length === 0 && (
                    <p className={classes.noProgressMsg}>אין נתוני התקדמות לתרגיל זה</p>
                  )}

                  {progressLoading && <p className={classes.noProgressMsg}>טוען...</p>}
                </>
              )}
            </div>
          </div>
