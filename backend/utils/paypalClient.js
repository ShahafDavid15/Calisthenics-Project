...
/**
 * Currency for Orders API — must match the JS SDK `currency` param.
 * Default ILS to match membership prices (₪) in the app.
 * If Sandbox returns CURRENCY_NOT_ALLOWED, set PAYPAL_CURRENCY=USD in .env for local testing.
 */
function getPayPalCurrency() {
  const explicit = (process.env.PAYPAL_CURRENCY || "").trim().toUpperCase();
  if (explicit) return explicit;
  return "ILS";
}

module.exports = { client, getPayPalCurrency };
